...
wget https://github.com/JeversonDiasSilva/fbalpha2012/releases/download/V1.1/FBA1 > /dev/null 2>&1
wget https://github.com/JeversonDiasSilva/fbalpha2012/releases/download/V1.1/FBA2 > /dev/null 2>&1
sleep 3

echo -e "${ROXO}INSTALANDO OS PACOTES...${RESET}"
unsquashfs -d fbalpha FBA1 > /dev/null 2>&1
unsquashfs -d fbalpha FBA2 > /dev/null 2>&1
sleep 3

echo -e "${ROXO}FINALIZANDO INSTALAÇÃO...${RESET}"
mv "/userdata/roms/fbalpha/.dep/es_systems_fbalpha.cfg" /userdata/system/configs/emulationstation > /dev/null 2>&1
chattr +i "/userdata/roms/fbalpha/.dep/es_systems_fbalpha.cfg"
